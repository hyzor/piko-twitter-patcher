...
if ($Help) {
    ...
    Write-Host ""
    Write-Host "Options:"
    Write-Host "  --output-dir DIR         Specify output directory (default: output)"
    Write-Host "  --cli-version VER       Specify ReVanced CLI version (default: 5.0.2-dev.2)"  
    Write-Host "  --patches-version VER    Specify Piko Patches version (default: auto - fetches latest from GitHub)"
    Write-Host "  --twitter-version VER    Specify Twitter version (default: latest)"
    Write-Host "  --help                 Show this help message"
    Write-Host ""
    Write-Host "Example:"
    Write-Host "  .\run.ps1"
    Write-Host "  .\run.ps1 --output-dir custom_output"
    Write-Host ""
    exit 0
}
